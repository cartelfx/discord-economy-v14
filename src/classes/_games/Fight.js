...
module.exports = async (options) => {
	...
	if (data.has(options.message.author.id) || data.has(options.opponent.id)) {
		return options.message.react("❎").catch(err => {});
	}
...
.setEmoji("❎");
			...
			const emd = new Discord.EmbedBuilder().setColor("Random")
			...
			let row = new Discord.ActionRowBuilder()
				...
				.addComponents(btn2)
				.addComponents(btn3);
			const _embed = new Discord.EmbedBuilder().setColor("Random")
					...
					if (!checkHealth(player)) {
						...
						if (msg.customId === id1) {
							...
							if (gameData[player].member.id == options.message.author.id) {
								const __embed = new Discord.EmbedBuilder().setColor("Random")
